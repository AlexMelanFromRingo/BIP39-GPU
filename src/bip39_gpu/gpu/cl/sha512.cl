/*
 * SHA-512 OpenCL kernel
 * Based on FIPS 180-4 specification
 * Optimized for BIP39 seed generation
 */
...
// Single SHA-512 hash
__kernel void sha512_single(
    __global const uchar *input,
    __global uint *input_length,
    ...
    uint msg_len = input_length[gid];
    __global const uchar *msg = input + gid * 256;  // Max 256 bytes per message

    // Process message in 128-byte blocks
    uchar block[128];
    uint blocks = (msg_len + 1 + 16 + 127) / 128;  // Padding + length
    ...
    for (uint blk = 0; blk < blocks; blk++) {
        ...
        // Copy message data
        uint block_start = blk * 128;
        uint block_end = min(block_start + 128, msg_len);

        for (uint i = block_start; i < block_end; i++) {
            block[i - block_start] = msg[i];
        }

        // Add padding
        if (blk == blocks - 1) {
            if (msg_len < block_end) {
                block[msg_len - block_start] = 0x80;
            }
            // Add length in bits (last 16 bytes, big-endian)
    ...
    // Output hash (64 bytes)
    __global uchar *out = output + gid * 64;
    for (int i = 0; i < 8; i++) {
